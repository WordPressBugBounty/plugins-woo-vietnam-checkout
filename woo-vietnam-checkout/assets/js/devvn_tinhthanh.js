    ...
    $(document).ready(function(){
        ...
        if($('#calc_shipping_city_field').length > 0){
            $( document.body ).bind( 'country_to_state_changed updated_wc_div country_to_state_changing', function() {
                ...
                var devvn_district_by_state = function(state_id){
                    ...
                    if(state_id && !loading_cacl) {
                        $.ajax({
                            ...
                            dataType: "json",
                            url: vncheckout_array.get_address,
                            data: {action: "load_diagioihanhchinh", matp: state_id},
                            context: this,
                            beforeSend: function () {
                                loading_cacl = true;
                            },
                            success: function (response) {
                                ...
                                if (response.success) {
                                    ...
                                    var data = [];
                                    $.each(listQH, function (index, value) {
                                        data.push({id:value.maqh,text:value.name});
                                    });
                ...
                var devvn_city_to_district_select2 = function() {
                    $( 'body select.state_select:visible' ).each( function() {
                        devvn_district_by_state($(this).val());
                        $(this, 'body').on('change select2:select', function () {
                            devvn_district_by_state($(this).val());
                        });
                    });
                };
                devvn_city_to_district_select2();
            });
        }
        //
                        ...
                        if (response.success) {
                            ...
                            if(response.data.billing.billing_state){
                                ...
                                if($("#billing_state").val()){
                                    ...
                                    var listQH = response.data.district;
                                    var newState = new Option('', '');
                                    $("#billing_city").append(newState);
                                    $.each(listQH,function(index,value){
                                        var newState = new Option(value.name, value.maqh);
                            ...
                            //thisParent.addClass('get_address_error');
                            //$('.btn_cancel', thisParent).html('Okie');
                        }
