...
if (
    ...
    if(!class_exists('Woo_Address_Selectbox_Class')) {
        class Woo_Address_Selectbox_Class
        {
            ...
            public function load_textdomain() {
                ...
                $locale = apply_filters( 'plugin_locale', $locale, 'woo-vietnam-checkout' );

                unload_textdomain( 'woo-vietnam-checkout' );
                load_textdomain( 'woo-vietnam-checkout', WP_LANG_DIR . '/plugins/woo-vietnam-checkout-' . $locale . '.mo' );
                load_plugin_textdomain( 'woo-vietnam-checkout', false, dirname(plugin_basename( __FILE__ )) . '/languages/' );
            ...
            function custom_override_checkout_fields($fields)
            {
                ...
                if (!$this->get_options('enable_firstname')) {
                    ...
                    $fields['billing']['billing_last_name'] = array(
                        'label' => __('Full name', 'woo-vietnam-checkout'),
                        'placeholder' => _x('Type Full name', 'placeholder', 'woo-vietnam-checkout'),
                        'required' => true,
                ...
                if (isset($fields['billing']['billing_phone'])) {
                    ...
                    $fields['billing']['billing_phone']['placeholder'] = __('Type your phone', 'woo-vietnam-checkout');
                }
                if (isset($fields['billing']['billing_email'])) {
                    $fields['billing']['billing_email']['class'] = array('form-row-last');
                    $fields['billing']['billing_email']['placeholder'] = __('Type your email', 'woo-vietnam-checkout');
                }
                $fields['billing']['billing_state'] = array(
                    'label' => __('Province/City', 'woo-vietnam-checkout'),
                    'required' => true,
                    'type' => 'select',
                    'class' => array('form-row-first', 'address-field', 'update_totals_on_change'),
                    'placeholder' => _x('Select Province/City', 'placeholder', 'woo-vietnam-checkout'),
                    'options' => array('' => __('Select Province/City', 'woo-vietnam-checkout')) + apply_filters('devvn_states_vn', $tinh_thanhpho),
                    'priority' => 30
                );
                ...
                if (!$this->get_options('enable_firstname')) {
                    $fields['shipping']['shipping_last_name'] = array(
                        'label' => __('Recipient full name', 'woo-vietnam-checkout'),
                        'placeholder' => _x('Recipient full name', 'placeholder', 'woo-vietnam-checkout'),
                        'required' => true,
                ...
                $fields['shipping']['shipping_phone'] = array(
                    'label' => __('Recipient phone', 'woo-vietnam-checkout'),
                    'placeholder' => _x('Recipient phone', 'placeholder', 'woo-vietnam-checkout'),
                    'required' => false,
                    'class' => array('form-row-last'),
                    'clear' => true,
                    'priority' => 20
            ...
            function devvn_enqueue_UseAjaxInWp()
            {
                ...
                if (($page_id && in_array(get_the_ID(), $page_id) ) || ($edit_address && is_page($edit_address)) || apply_filters('vn_checkout_allow_script_all_page', false)) {
                    ...
                    $php_array = array(
                        ...
                        'home_url' => home_url(),
                        'formatNoMatches' => __('No value', 'woo-vietnam-checkout'),
                        'phone_error' => __('Phone number is incorrect', 'woo-vietnam-checkout'),
                        'loading_text' => __('Loading...', 'woo-vietnam-checkout'),
                        'loadaddress_error' => __('Phone number does not exist', 'woo-vietnam-checkout')
                    );
            ...
            {
                echo '<p><label for="_shipping_phone">' . __('Phone number of the recipient', 'woo-vietnam-checkout') . ':</label> <br>
            ...
            function devvn_woocommerce_order_details_after_customer_details($order)
            {
                ...
                if ($sdtnguoinhan) : ?>
                    <tr>
                        <th><?php _e('Shipping Phone:', 'woo-vietnam-checkout'); ?></th>
            ...
            function devvn_modify_plugin_update_message($plugin_data, $response)
            {
                ...
                echo '<br />' . sprintf(__('<strong>Mua bản quyền để được tự động update. <a href="%s" target="_blank">Xem thêm thông tin mua bản quyền</a></strong> hoặc liên hệ mua trực tiếp qua <a href="%s" target="_blank">facebook</a>', 'woo-vietnam-checkout'), $PluginURI, 'http://m.me/levantoan.wp');
            }
    ...
    if(!function_exists('devvn_round_up')) {
