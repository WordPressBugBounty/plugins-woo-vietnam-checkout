		...
		<table class="form-table">
			<tbody>
                ...
                <tr>
                    ...
                    <td>
                        <input type="number" min="0" name="<?php echo $this->_optionName?>[khoiluong_quydoi]" value="<?php echo esc_attr($flra_options['khoiluong_quydoi']);?>" id="khoiluong_quydoi"/> <br>
                        ...
                        <select name="<?php echo $this->_optionName?>[vnd2usd_currency]" id="vnd2usd_currency">
                            ...
                            foreach ( $paypal_supported_currencies as $currency ) {
                                if ( strtoupper( $currency ) == $flra_options['vnd2usd_currency'] ) {
                                    printf( '<option selected="selected" value="%1$s">%1$s</option>', esc_attr($currency) );
                                } else {
                                    printf( '<option value="%1$s">%1$s</option>', esc_attr($currency) );
                                }
                            }
                            ?>
                        </select>
                ...
                <tr>
                    ...
                    <td>
                        <input type="number" min="0" name="<?php echo $this->_optionName?>[vnd_usd_rate]" value="<?php echo esc_attr($flra_options['vnd_usd_rate']);?>" id="vnd_usd_rate"/> <br>
                        <small><?php _e('Tỷ giá quy đổi từ VNĐ','devvn')?></small>
                    </td>
                </tr>
                <tr>
                    <th scope="row"><label for="alepay_support"><?php _e('Alepay','woo-vietnam-checkout')?></label></th>
                    <td>
                        <label><input type="checkbox" name="<?php echo $this->_optionName?>[enable_firstname]" <?php checked('1',$flra_options['enable_firstname'])?> value="1" /> <?php _e('Hiển thị trường First Name','woo-vietnam-checkout')?></label>
                ...
                <tr>
                    <th scope="row"></th>
                    <td>
                        <label><input type="checkbox" name="<?php echo $this->_optionName?>[enable_country]" <?php checked('1',$flra_options['enable_country'])?> value="1" /> <?php _e('Hiển thị trường Quốc Gia','woo-vietnam-checkout')?></label>
                ...
                <tr>
                    <th scope="row"></th>
                    <td>
                        <label><input type="checkbox" name="<?php echo $this->_optionName?>[enable_postcode]" <?php checked('1',$flra_options['enable_postcode'])?> value="1" /> <?php _e('Hiện trường Postcode','woo-vietnam-checkout')?></label>
                        <br><small>Nếu sử dụng kiểu thanh toán "Tokenization" của Alepay thì bắt buộc cần Postcode.</small>
                    </td>
                </tr>
                <tr style="color: #9f9f9f;">
                    <th scope="row" style="color: #9f9f9f;"><label for="enable_getaddressfromphone"><?php _e('Lấy địa chỉ tự động','woo-vietnam-checkout')?></label></th>
                    <td>
                        <label><input type="checkbox" name="<?php echo $this->_optionName?>[enable_getaddressfromphone]" <?php checked('1',$flra_options['enable_getaddressfromphone'])?> value="1" /> <?php _e('Lấy địa chỉ tự động','woo-vietnam-checkout')?></label>
                        <br><small>Chức năng này cho phép nhập SĐT để lấy địa chỉ đã có của khách hàng.</small>
                        <?php vn_checkout_up_to_pro();?>
                    </td>
                </tr>
                <tr style="color: #9f9f9f;">
                    <th scope="row" style=" color: #9f9f9f; "><label for="enable_recaptcha"><?php _e('Cấu hình ReCaptcha','woo-vietnam-checkout')?></label></th>
                    <td>
                        <label><input type="checkbox" name="<?php echo $this->_optionName?>[enable_recaptcha]" <?php checked('1',$flra_options['enable_recaptcha'])?> value="1" /> <?php _e('Sử dụng recaptcha','woo-vietnam-checkout')?></label>
                        <br><small>Chức năng này sẽ hiện captcha khi lấy địa chỉ bằng số điện thoại đặt hàng trước đó. Tránh bot spam.</small>
                        <?php vn_checkout_up_to_pro();?>
                    </td>
                </tr>
                <tr>
                    <th scope="row"><label for="active_filter_order"><?php _e('Bộ lọc','woo-vietnam-checkout')?></label></th>
                    <td>
                        <label><input type="checkbox" name="<?php echo $this->_optionName?>[active_filter_order]" <?php checked('1',$flra_options['active_filter_order'])?> value="1" /> <?php _e('Kích hoạt bộ lọc','woo-vietnam-checkout')?></label>
